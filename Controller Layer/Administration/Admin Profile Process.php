...
<?php 
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update-profile']) && isset($_GET['admin'])){
        ...
        $sql = "UPDATE admin SET AdminName='$username', `AdminEmail`='$email', `AdminContact`='$phone' WHERE AdminId = '$id'";
    ...
    // update password
    if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update-password']) && isset($_GET['admin'])){
        ...
        if($password !== $conPassword){
            echo "<script>
                alert('Not matching');
                window.location.href = ' ../../View Layer/Administration/Admin Account.php';
            </script>";
        }
        else {
            // validate password security
            $sqlSearch = "SELECT * FROM admin WHERE AdminId = $id AND AdminPassword = $oldPassword";

            $resultValid = mysqli_query($con, $sqlSearch);

            if(mysqli_num_rows($resultValid) === 1){
                $sql = "UPDATE admin SET `AdminPassword`= '$password' WHERE AdminId = '$id'";
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update-image']) && isset($_GET['admin'])){

        $id = $_GET['admin'];

        // File upload handling
        $targetDirectory = "../../Assets/Profile-Image/Administration/";
